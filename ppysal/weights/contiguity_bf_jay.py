...
This is utilizing the built-in multiprocess module and leveraging
Serge's vectorization work.
...
"""
_author_ = "Serge Rey <sjsrey@gmail.com>, Jay Laura <jlaura@asu.edu"
...
if __name__ == '__main__':
    ...
    varc_rows_2_polys = mp.RawArray(ctypes.c_int, max_g[2]*n*2)
    rows_2_polys = np.frombuffer(varc_rows_2_polys)
    rows_2_polys[:] = 0
    rows_2_polys.shape = (max_g[2], n)

    varc_cols_2_polys = mp.RawArray(ctypes.c_int, max_g[3]*n*2)
    cols_2_polys = np.frombuffer(varc_cols_2_polys)
    cols_2_polys[:] = 0
    ...
    #Jagged ararys with ctypes an no memory duplication might not be possible
    # This version uses nan for empty indices
    # The size difference (jagged | full) 80563 | 518280
    #Each row is twice the length of the maximum number of vertices
    #Even indices are X coords, odd indices are Y coords.
    c_vert = mp.RawArray(ctypes.c_double, max_verts * n * 2)
    vertices = np.frombuffer(c_vert)
    vertices[:] = np.nan
    vertices.shape = (n,max_verts * 2)
    #A cleaner way must exist to do thi by slicing the list of tuples, ideas?
    for i, shp in enumerate(shps):
        c = 0
        for j,v in enumerate(shp.vertices):
            vertices[i][j+c] = v[0]
