...
void tiffIO::write(long xstart, long ystart, long numRows, long numCols, void* source, int sourceStride) {
    ...
    const char *extension_list[7] = {".tif", ".img", ".sdat", ".bil", ".bin", ".tiff", ".vrt"}; // extension list --can add more 
    const char *driver_code[7] = {"GTiff", "HFA", "SAGA", "EHdr", "ENVI", "GTiff", " "}; //  code list -- can add more
    const char *compression_meth[7] = {"LZW", "YES", " ", " ", " ", " ", " "}; //  code list -- can add more
    size_t extension_num = 7;
    char *ext;
    int index = -1;
    ...
    // if the output is vrt file, each process write its own portion
    if (index == 6) {
        // write uncompressed tiff files by default
