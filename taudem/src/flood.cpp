        ...
        bool is_4Point,bool use_mask,char *maskfile)  // these three added by arb, 5/31/11
{
    ...
    MPI_Init(NULL,NULL);{
        ...
        std::unique_ptr<linearpart<short>> maskPartition = NULL;
        if (use_mask)
            maskPartition = std::unique_ptr<linearpart<short>>(new linearpart<short>(totalX, totalY, dxA, dyA, MPI_SHORT, *(short *) depmask->getNodata()));
        ...
        linearpart<float> planchon(totalX, totalY, dxA, dyA, MPI_FLOAT, felNodata);
        ...
        {
            printf("Data read\n");
            float val = elevDEM.getData(nx / 2, ny / 2);
        ...
        for(int j=0; j<ny; j++){
            for(int i=0; i<nx; i++){
                ...
                else if (use_mask && maskPartition->getData(i,j)==1) // logic for setting the elevation when using a depression mask
                    planchon.setData(i,j,elevDEM.getData(i,j));
        ...
        while(planchon.isInPartition(i,j)) { 
            ...
            if(!planchon.isNodata(i,j) && planchon.getData(i,j) > elevDEM.getData(i,j)){
                //Checks each direction...
                float neighborFloat = FLT_MAX;
                for(k=1; k<=8; k+=step){
                    in = i+d1[k];
                    jn = j+d2[k];
                    if(planchon.hasAccess(in,jn) && planchon.getData(in,jn) < neighborFloat)
                        //Get neighbor data and store as planchon for self
                        neighborFloat = planchon.getData(in,jn);
                }
                ...
                if(elevDEM.getData(i,j) >= neighborFloat){
                    planchon.setData(i,j, elevDEM.getData(i,j));
                    finished = false;  
                }
                ...
                {	
                    ...
                    if(planchon.getData(i,j) > neighborFloat /* && elevDEM->getData(i,j,tempFloat) < neighborFloat */){
        ...
        while(!finished){
            ...
            if(verbose)
            {
                ...
                float midpointVal = planchon.getData(nx/2,ny/2);
                printf("Result midpoint of partition: %d, nxm: %d, nym: %d, value: %f\n",rank,nx/2,ny/2, midpointVal);
                fflush(stdout);
            }
